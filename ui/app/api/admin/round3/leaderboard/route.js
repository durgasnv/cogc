...
export async function GET(req) {
  ...
  const store = kv();
  let teamIds = (await store.smembers('teams:index')) || [];
  if (!teamIds.length) {
    teamIds = (await store.smembers('teams')) || [];
  }
  const threshold = (await store.get('round3:threshold')) ?? 1200; // default 12/20 = 1200 pts
  const revealed = !!(await store.get('round3:revealed'));

  const rows = [];
  for (const id of teamIds) {
    const team = await store.get(`team:${id}`);
    const score = await store.get(`round3:score:${id}`);
  ...
  // Sort rows with strict tie-breaking:
...
export async function POST(req) {
  ...
  if (typeof body.threshold === 'number') {
    await store.set('round3:threshold', body.threshold);
  }
  if (typeof body.revealed === 'boolean') {
    await store.set('round3:revealed', body.revealed);
  }
  if (body.reset) {
    let teamIds = (await store.smembers('teams:index')) || [];
    if (!teamIds.length) teamIds = (await store.smembers('teams')) || [];
    for (const id of teamIds) {
      await store.del(`round3:score:${id}`);
    }
    await store.del('round3:participants');
  }
